...
const SubscriptionPlans = () => {
  ...
  const fetchPlans = async () => {
    try {
      const response = await axios.get('/api/v1/subPlan/all');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editingId) {
        const ed = await axios.put(`/api/v1/subPlan/update/${editingId}`, formData);
        if (ed.data.success) {
        ...
        const cd = await axios.post('/api/v1/subPlan/create', formData);
        if (cd.data.success) {
  ...
  const handleEdit = (plan) => {
    setFormData({
      ...
      price: plan.price,
      planStatus: plan.planStatus, // Ensure this is correctly set
    });
  ...
  const handleDelete = async (id) => {
    try {
      const dt = await axios.delete(`/api/v1/subPlan/delete/${id}`);
      if (dt.data.success) {
  ...
  const handleClose = () => {
    ...
    setFormData({
      ...
      duration: '',
      price: '',
      planStatus: 'Active',  // Reset to default
    });
    setEditingId(null);
  };
        ...
        <Grid container spacing={2}>
          {plans.map((plan) => (
            <Grid item xs={12} sm={6} md={4} lg={3} xl={2} key={plan._id}>
              <Card variant="outlined" sx={{ mb: 2, pt: 0.2, pl: 2 }}>
                <CardContent align="left">
                  <Typography variant="h6" pb={1} sx={{ pb: 1, background: 'purple', textAlign:"center", color: 'white' ,borderRadius: '5px',  padding: '2px 6px'}}>
                   {plan.planName}  Plan 
                  </Typography>
                  <Typography>Duration: {plan.duration} days</Typography>
                  <Typography variant="h5"   sx={{textAlign:"center", color:"tomato"  }}> Price ₹{plan.price}</Typography>
