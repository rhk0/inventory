...
const SubscribedUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/api/v1/super/subscribed-users'); // Replace with your API URL
        
        // Extracting subscribed users from the response data
        const subscribedUsers = response.data.users;

        // Mapping subscribed users to include businessType
        const mappedRows = subscribedUsers?.map(user => ({
          id: user._id,
